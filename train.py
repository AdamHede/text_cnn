...
# Load data
print("Loading data...")
x, y, vocabulary, vocabulary_inv, real_codes = data_helpers.load_data()
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_step(x_batch, y_batch):
            ...
            _, step, summaries, loss, accuracy = sess.run(                                      ##Where everything comes together
        ...
        def prediction_step(x_full_batch, y_full_batch, writer=None):
            """
            Attempts prediction
            """
            feed_dict = {
              cnn.input_x: x_full_batch,
              cnn.input_y: y_full_batch,
              cnn.dropout_keep_prob: 1.0
            }
            step, summaries, loss, accuracy = sess.run(
                [global_step, dev_summary_op, cnn.loss, cnn.accuracy],
            ...
            classification = sess.run(cnn.predictions, feed_dict)
            print classification
            ##true_codes = vocabulary
            file_out = zip(real_codes, classification)
            data_helpers.write_predictions(file_out)
            print("{}: step {}, loss {:g}, acc {:g}".format(time_str, step, loss, accuracy))
            if writer:
                writer.add_summary(summaries, step)
        ...
        for batch in batches:
            ...
            if current_step % FLAGS.attempt_predictions_every == 0:
                print("\nFull prediction set")
                prediction_step(x_full, y_full)
                print("")
